<script lang="ts">
	...
	const breadcrumbSchema = $derived(
		generateBreadcrumbSchema([
			{ name: $t('nav.home'), url: `/${$page.params.locale}` },
			{ name: $t('vehicle.cars'), url: `/${$page.params.locale}/xe/oto` },
...
<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
	...
	<nav class="mb-6 text-sm" aria-label="Breadcrumb">
		<ol class="flex items-center gap-2">
			...
			<li><a href="/{$page.params.locale}/xe/oto" class="text-gray-500 hover:text-gray-700">{$t('vehicle.cars')}</a></li>
			<li class="text-gray-400">/</li>
	...
	<div class="lg:grid lg:grid-cols-2 lg:gap-12">
		...
		<div>
			...
			<div class="mt-8 flex gap-4">
				<a href="/{$page.params.locale}/so-sanh?cars={car.id}" class="btn-primary flex-1 text-center">{$t('car.addToCompare')}</a>
				<a href="/{$page.params.locale}/tinh-gia?car={car.id}" class="btn-secondary flex-1 text-center">{$t('nav.calculator')}</a>
			</div>
		</div>
	</div>
