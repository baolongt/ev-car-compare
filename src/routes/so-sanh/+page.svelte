...
<svelte:head>
	<title>{pageTitle('So sánh xe điện')}</title>
	<meta
		name="description"
		content="So sánh thông số kỹ thuật và giá cả giữa các mẫu xe điện. Chọn tối đa 3 xe để so sánh chi tiết."
	/>
</svelte:head>
...
<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
	...
	<div class="flex flex-wrap items-start justify-between gap-4">
		<div>
			<h1 class="text-3xl font-bold text-gray-900">So Sánh Xe Điện</h1>
			<p class="mt-2 text-gray-500">
				Chọn từ 2 đến {MAX_COMPARE_CARS} xe để so sánh thông số kỹ thuật chi tiết.
			</p>
		</div>
		...
		{#if $comparisonCars.length >= 2}
			<div class="flex items-center gap-3">
				<label class="flex items-center gap-2 text-sm">
					...
					<span class="text-gray-600">Đánh dấu khác biệt</span>
				</label>
				<button
				...
				>
					...
					{showCopySuccess ? 'Đã sao chép!' : 'Chia sẻ'}
				</button>
				<button
	...
	{:else}
		<div class="mt-12 text-center">
			...
			<h3 class="text-lg font-medium text-gray-900">Chưa có xe nào được chọn</h3>
			<p class="mt-1 text-gray-500">Sử dụng ô tìm kiếm ở trên để thêm xe vào so sánh.</p>
	...
	{#if $comparisonCars.length === 1}
		<div class="mt-6 rounded-lg bg-blue-50 p-4 text-sm text-blue-700">
			<p>Thêm ít nhất 1 xe nữa để bắt đầu so sánh thông số kỹ thuật.</p>
		</div>
	{/if}
</div>
